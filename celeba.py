...
class _CelebA(VisionDataset):
    ...
    base_folder = "img_align_celeba"#"celeba"
    file_list = [
    ...
    def __init__(
        ...
        splits = pandas.read_csv(fn(data_dir + "list_eval_partition.csv"), header=0, index_col=0)
        attr = pandas.read_csv(fn(data_dir + "list_attr_celeba.csv"), header=0, index_col=0)
        ...
        print('---CELEB:')
        print('Num attr' , len(self.attr_names))
        print('Num attr' , self.attr_names)
        self.task = task
        if task:
...
def get_dataset(train_transform,test_transform):
    train_data = _CelebA(split="train", transform = train_transform)
    val_data = _CelebA(split="valid", transform = test_transform)
    test_data = _CelebA(split="test", transform = test_transform)
